...
var health_scale: float = 81
var health_render_scale: float = 0.0
var is_dead: bool = false
@onready var unscaled_health : float = get_parent().get_parent().get_parent().health
...
func render_healthbar():
	unscaled_health = get_parent().get_parent().get_parent().health if "health" in get_parent().get_parent().get_parent() else unscaled_health
